...
const pickSound = new Audio("menu.mp3");
...
if(window.matchMedia("(min-width: 377px) and (max-width: 769px)").matches == true) {
    game.addEventListener("click", (e) => {
        if (!stopflag) {
          pauseCharacter.classList.add("jump");
          stopflag = true;
...
if(window.matchMedia("(min-width: 377px) and (max-width: 426px)").matches == true) {
    ...
    // 연속으로 캐릭터가 뛰지 않도록 조절
      ...
      setInterval(() => {
        ...
        // 게임오버
        if (pauseCharacterTop > 70 && bombLeft > -135  && bombLeft < -38) {
          stopflag = true;
          gameoverSound.loop = false;
          gameoverSound.volume = 1;
          gameoverSound.play();
